...
以下数据来自本地 gold dataset 评测快照。样本规模为 125 条，包含文件、页码、chunk/root、关键词和 hard negative 信息。数据适合说明当前能力边界，但后续更换模型、重建索引、修改 chunk 规则或开启缓存后都需要重新评测。

### 推荐档位对比

评测口径：gold dataset，`RERANK_CACHE_ENABLED=false`，跳过重建索引，使用当前 `v3_quality` collection。

| 规范档位 | 历史别名 | dtype | File@5 | File+Page@5 | Chunk@5 | Root@5 | P50 | P95 | 结论 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `K2/I2/M0/A1/fp16` | `V3Q` | fp16 | 0.992 | 0.768 | 0.720 | 0.793 | 1116 ms | 1283 ms | 推荐默认质量档 |
| `K3/I2/M0/A1/fp16` | `V3Q_OPT` | fp16 | 0.968 | 0.704 | 0.673 | 0.713 | 718 ms | 816 ms | 快速备选档 |
| `K2/I2/M0/A1/bf16` | `V3Q` | bf16 | 0.984 | 0.768 | 0.720 | 0.793 | 1051 ms | 1190 ms | 可灰度观察 |
| `K2/I2/M0/A1/fp32` | `V3Q` | fp32 | 0.992 | 0.768 | 0.720 | 0.793 | 4483 ms | 5118 ms | 只适合调试或数值基线 |

解释：

- `K2/fp16` 在文件召回、页码定位、chunk 和 root 命中上最均衡，是当前默认口径。
- `K3/fp16` 牺牲一部分证据命中，换来约 35% 的 P50 延迟下降。
- `bf16` 的质量接近 `fp16`，但 File@5 略低，适合作为硬件灰度选项。
- `fp32` 没有带来明显质量收益，延迟显著升高，不适合在线默认。

### 在线 graph 口径

评测口径：gold dataset，`--mode graph`，不包含最终外部 LLM 答案生成。

| 规范档位 | 历史别名 | File@5 | File+Page@5 | Chunk@5 | Root@5 | P50 | P95 | 结论 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `K2/I2/M0/A1/fp16` | `V3Q` | 0.992 | 0.768 | 0.720 | 0.793 | 1148 ms | 1306 ms | graph 链路与离线 retrieval 基本一致 |
| `K3/I2/M0/A1/fp16` | `V3Q_OPT` | 0.968 | 0.704 | 0.673 | 0.713 | 740 ms | 850 ms | 可作为低延迟在线档 |

### 历史 v3.1 全量对比

早期 v3.1 做过 8 个变体、共 1000 行结果的全量对比。旧名只作为历史别名保留，当前默认口径以 `K/I/M/A/dtype` 为准。

| 历史变体 | File@5 | File+Page@5 | Chunk@5 | Root@5 | ChunkMRR | RootMRR | P50 | P95 | 结论 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `GB0` | 0.744 | 0.504 | 0.000 | 0.529 | 0.000 | 0.392 | 939 ms | 1061 ms | 基线完整，但证据 chunk 弱 |
| `GS1` | 0.744 | 0.504 | 0.000 | 0.529 | 0.000 | 0.392 | 823 ms | 1038 ms | 质量与 GB0 接近 |
| `GS2` | 0.872 | 0.680 | 0.000 | 0.644 | 0.000 | 0.508 | 847 ms | 1058 ms | QueryPlan 提升文件、页码和 root |
| `GS2H` | 0.864 | 0.616 | 0.000 | 0.609 | 0.000 | 0.480 | 836 ms | 930 ms | 标题词法增强对页码定位不稳定 |
| `GS2HR` | 0.992 | 0.752 | 0.000 | 0.770 | 0.000 | 0.645 | 1532 ms | 2060 ms | 文件和 root 强，但 chunk ID 未对齐 |
| `GS3` | 0.920 | 0.712 | 0.655 | 0.724 | 0.580 | 0.637 | 1144 ms | 1610 ms | 早期最佳均衡基线 |
| `V3Q` | 0.992 | 0.768 | 0.724 | 0.793 | 0.575 | 0.635 | 3938 ms | 4450 ms | 早期质量上限，延迟偏高 |
| `V3F` | 0.416 | 0.280 | 0.276 | 0.310 | 0.210 | 0.228 | 34 ms | 3047 ms | profile/index 不一致，不适合生产 |

这组数据推动了后续设计取舍：SuperHermes 没有继续盲目堆更复杂的生成策略，而是优先把文件召回、页码定位、chunk/root 证据、qrel 口径和 trace 诊断做扎实。
